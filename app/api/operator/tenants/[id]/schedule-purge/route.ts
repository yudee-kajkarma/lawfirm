...
export const POST = withOperatorAuth<Params>(async (req, { params }, { operator }) => {
  ...
  if (tenant.status === 'active') {
    return apiError('CONFLICT', 'Tenant must be suspended before scheduling purge', 409);
  }
  if (tenant.status === 'pending_purge' || tenant.status === 'purging') {
    return apiError('CONFLICT', 'Tenant is already scheduled for purge', 409);
  }

  // Valid transition: suspended → pending_purge
  // purgeScheduledAt = suspendedAt (or now if somehow null) + 30 days
  const baseTime = tenant.suspendedAt ?? new Date();
  const purgeScheduledAt = new Date(baseTime.getTime() + 30 * 24 * 60 * 60 * 1000);

  tenant.status = 'pending_purge';
  tenant.purgeScheduledAt = purgeScheduledAt;
  await tenant.save();
  ...
  await writeOperatorAudit({
    ...
    details: { purgeScheduledAt: purgeScheduledAt.toISOString() },
